...
for ($row = 2; $row <= $highestRow; $row++) {
    ...
    if ($issues) {
        if ($incidencia === null) {
            ...
            if (!empty($prioridadGhi)) $comando .= " -L $prioridadGhi";
            if (!empty($tipoGhi)) $comando .= " -L $tipoGhi";
            if (!empty($complejidadGhi)) $comando .= " -L $complejidadGhi";
            if (!empty($entregaGhi)) $comando .= " -M $entregaGhi";
